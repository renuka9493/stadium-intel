...
const stands = [
  { name: "Al Baik Express", cuisine: "Broasted Chicken", wait: 4, rating: 4.8, zone: "Concourse 2" },
  { name: "Machboos Corner", cuisine: "Qatari Rice", wait: 9, rating: 4.6, zone: "Concourse 3" },
  { name: "Pitch Perfect Burgers", cuisine: "American", wait: 12, rating: 4.4, zone: "North" },
  { name: "Green Falcon Vegan", cuisine: "Plant-based", wait: 3, rating: 4.9, zone: "South" },
  { name: "Tea & Karak", cuisine: "Beverages", wait: 2, rating: 4.7, zone: "Everywhere" },
  { name: "Pizza 90'", cuisine: "Italian", wait: 15, rating: 4.3, zone: "East" },
];
        ...
        {stands.map((s) => (
          <div key={s.name} className="rounded-2xl border border-border/60 bg-card p-5 shadow-card transition hover:-translate-y-0.5 hover:border-accent/40">
            ...
            <div className="mt-4 flex items-center justify-between text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> {s.wait} min wait</span>
